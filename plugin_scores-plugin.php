...
class plugin_scores_class  extends basic_plugin_class {
	...
	const RSS_URL      = 'https://wordpress.org/support/plugin/';
	//                    https://wordpress.org/support/plugin/gps-map-widget/
	...
	*/
	function showPluginInfo($pluginSlug) {
		...
		$rssUrl = self::RSS_URL.$pluginSlug.'/';
		$rss = fetch_feed($rssUrl.'feed');
		$num_support= $rss->get_item_quantity();
		...
		if ($num_support) {
			$res .= '<td class="center"><a href="'.$rssUrl.'">'.$num_support.'</a></td>';
		} else {
			$res .= '<td class="center faded">'.$num_support.'</td>';
		}
	...
	function plugin_scores_create_widget_function() {
		...
		$res .=  '  <th class="center">'.__('No.Threads',self::FS_TEXTDOMAIN).'</th>';
		$res .=  "</tr></thead>";
